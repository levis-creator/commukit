...
AudioBridge moderation is **server-enforced** — unlike SFU-based audio,
the Janus mixer owns the mix and can genuinely silence a participant.
A muted client can still send audio upstream, but the server drops
their contribution from the mix.
...
AudioBridge yet — your backend should retry with backoff for
...
Removes the participant from the Janus AudioBridge room. They can
technically reconnect via `authorize-user` unless you also invalidate
the session.
...
Use this to render a moderation UI.
...
|---|---|---|
| **Client-side** | The user themselves, via `configure: { muted: true }` on their own handle | "I want to mute myself" |
...
actor) plus a JSON metadata blob with resolved Janus participant IDs.
